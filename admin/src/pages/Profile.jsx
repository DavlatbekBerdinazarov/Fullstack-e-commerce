...
export default function AddAdmin() {
  const { admin, profImg, adminId } = useContext(AdminPanelContext);
  ...
  const onSubmitHandler = (e) => {
    ...
    axios
      .put(`http://localhost:4545/api/register-update/${adminId}`, formdata, {
         headers: {
    ...
    <div className="w-full pl-5 pt-5 ">
      <Card className="h-[85vh] xl:w-3/4 py-8 px-16 bg-[#f5faff]">
        ...
        <form onSubmit={onSubmitHandler} className="flex gap-12 w-full">
          <div>
            <div className="w-36 min-h-40 rounded-md">
              <img
                className="w-36 min-h-40 rounded-md shadow-lg object-cover"
                src={ profImg ? profImg : `https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_640.png`}
                alt="img here"
              />
            </div>
            <div>
